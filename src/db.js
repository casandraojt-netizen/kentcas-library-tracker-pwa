...
export function setNeonUrl(url) { localStorage.setItem(STORAGE_KEY, url.trim()) }
export function getNeonUrl() { return localStorage.getItem(STORAGE_KEY) || '' }
export function clearNeonUrl() { localStorage.removeItem(STORAGE_KEY) }
export function hasNeonUrl() { return !!getNeonUrl() }
...
export async function upsertBook(book) {
  ...
  await runQuery(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      rss_last_item_title=EXCLUDED.rss_last_item_title, rss_last_item_date=EXCLUDED.rss_last_item_date,
...
export async function updateBook(id, updates) {
  ...
  await runQuery(`UPDATE books SET ${setClauses} WHERE id = $1`, [id, ...fields.map(f => updates[f])])
}

export async function deleteBook(id) {
  await runQuery(`UPDATE books SET deleted = true, updated_at = $2 WHERE id = $1`, [id, new Date().toISOString()])
}
